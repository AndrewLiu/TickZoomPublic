...
namespace TickZoom.TickUtil
{
	...
	public class TickWriterDefault : TickWriter
	{
		...
		public void Initialize(string folderOrfile, string _symbol) {
            tickFile.Initialize(folderOrfile, _symbol, BinaryFileMode.Write);
            if (!CancelPending)
            {
				StartAppendThread();
			}
		...
		protected virtual Yield Invoke()
		{
			...
			try {
				...
				if( ex.EntryType == EventType.Terminate) {
                    ...
                    if( debug) log.DebugFormat(LogMessage.LOGMSG382);
					Finish();
        ...
        public void Flush()
        {
            if( debug) log.DebugFormat(LogMessage.LOGMSG383, writeQueue.Count);
            tickFile.Flush();
            if (debug) log.DebugFormat(LogMessage.LOGMSG384, writeQueue.Count);
        }
			...
			if (!isDisposed) {
				...
				if( appendTask != null) {
					if( writeQueue != null) {
                        try
                        {
                            if (debug) log.DebugFormat(LogMessage.LOGMSG385, EventType.Terminate);
        ...
        private void Finish()
        {
            if (debug) log.DebugFormat(LogMessage.LOGMSG386);
            Flush();
            var count = tickFile.WriteCounter;
            var append = Interlocked.Read(ref appendCounter);
            if (debug) log.DebugFormat(LogMessage.LOGMSG387, count, append);
            if (tickFile != null)
            {
                tickFile.Dispose();
            }
