...
namespace TickZoom.Provider.FIX
{
    public class SocketReconnect : IDisposable, LogAware
    {
        ...
        {
            if (debug) log.DebugFormat(LogMessage.LOGMSG300);
            Socket old = socket;
            if (socket != null)
            {
                socket.Dispose();
                switch (socket.State)
                {
                    ...
                    case SocketState.PendingConnect:
                        if (debug) log.DebugFormat(LogMessage.LOGMSG301, socket.State);
                        return;
            ...
            while (true)
            {
                ...
                {
                    socket.Connect();
                    if (debug) log.DebugFormat(LogMessage.LOGMSG302, socket);
                    SetupRetry();
                    return;
                }
