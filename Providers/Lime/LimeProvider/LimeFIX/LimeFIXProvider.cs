       ...
       protected override void OnFinishRecovery() 
		...
		protected override void ReceiveMessage(Message message) {
			...
			switch( packetFIX.MessageType) {
                case "8":
                    if (trace) log.TraceFormat("Received Execution Report");
                    ...
                    break;
                case "9":
                    if (trace) log.TraceFormat("Received Cancel Rejected");
                    CancelRejected(packetFIX);
                    break;
                case "j":
                    if (trace) log.TraceFormat("Received Business Reject");
        ...
        private void TrySetOrderServerOnline() {
            if ( !isOrderServerOnline )
            {
                ...
                if (debug) log.DebugFormat("Setting Order Server online");
                CancelRecovered();
                TrySendEndBroker();
                TryEndRecovery();
            }
        ...
        private void ExecutionReport( MessageFIX4_2 packetFIX)
		{
            ...
            if (debug) log.DebugFormat("Resulting orders in snapshot: \n{0}", OrderStore.OrdersToString());
        }
